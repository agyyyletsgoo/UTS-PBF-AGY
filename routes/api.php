...
Route::middleware(['jwt-product-auth'])->group(function () {
    Route::post("/products", [ProductControll::class, "store"]);
    Route::get("/products", [ProductControll::class, "ShowAll"]);
    Route::put("/products/{id}", [ProductControll::class, "update"]);
    Route::delete("/products/{id}", [ProductControll::class, "delete"]);
});
